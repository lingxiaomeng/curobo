...
.. code-block:: bash
   ...
   python -m curobo_floating_base.examples.getting_started.motion_planning
...
.. code-block:: bash
   ...
   python -m curobo_floating_base.examples.getting_started.motion_planning --mode pose
   python -m curobo_floating_base.examples.getting_started.motion_planning --mode grasp
...
When the tutorial finishes successfully you will see::
    ...
    Duration: 5.00s
    Trajectory plot saved to: ~/.cache/curobo_floating_base/examples/motion_planning/motion_plan.pdf
    ...
    ✓ Grasp planning succeeded!
      ...
      Grasp:    45 waypoints
      Lift:     60 waypoints
    Trajectory plot saved to: ~/.cache/curobo_floating_base/examples/motion_planning/grasp_plan.pdf

Output files are written to ``~/.cache/curobo_floating_base/`` by default. Override with
``--output-dir`` or by setting ``curobo_floating_base._src.runtime.cache_dir`` in Python.
...
.. code-block:: bash
   ...
   python -m curobo_floating_base.examples.getting_started.motion_planning --visualize
...
from curobo_floating_base.types import ContentPath, GoalToolPose, JointState, Pose
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for output files (default: ~/.cache/curobo_floating_base/examples/motion_planning/)",
    )
