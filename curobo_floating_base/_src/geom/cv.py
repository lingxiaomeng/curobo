...
def extract_depth_from_structured_pointcloud(
    ...
    """Extract depth values from structured pointcloud in camera frame.

    This function assumes the pointcloud maintains a spatial grid structure [b, h, w, 3]
    and is in camera frame where Z-axis is aligned with depth. It simply extracts
    the Z-coordinates without performing actual camera projection.

    **Important:** This is NOT a general pointcloud-to-image projection function.
    It only works for structured pointclouds that maintain image topology.

    Args:
        pointcloud: Structured pointcloud in camera frame. Shape: (b, h, w, 3).
        output_image: Pre-allocated output tensor. Shape: (b, h, w).

    Returns:
        torch.Tensor: Depth image extracted from Z-coordinates. Shape: (b, h, w).
    """
    batch, width, height = output_image.shape

    output_image = output_image.view(batch, -1)
    output_image[:] = pointcloud[:, :, :, 2].reshape(batch, -1)
    output_image = output_image.view(batch, width, height)
    return output_image
...
def project_depth_using_rays(
    ...
    Projection rays can be calculated using :func:`~curobo_floating_base.geom.cv.get_projection_rays` function.
