...
Most of these implementations are available through :class:`~curobo_floating_base.types.pose.Pose`.
Most of these implementations are available through :class:`~curobo_floating_base.types.pose.Pose`.
"""
...
# CuRobo
from curobo_floating_base._src.curobolib.cuda_ops.tensor_checks import check_float32_tensors
from curobo_floating_base._src.util.logging import log_and_raise
from curobo_floating_base._src.util.torch_util import get_torch_jit_decorator
from curobo_floating_base._src.util.warp import get_warp_device_stream, init_warp
...
def quaternion_rate_to_axis_angle_rate(
    ...
    qw = current_quaternion[..., 0]
    qx = current_quaternion[..., 1]
    qy = current_quaternion[..., 2]
    qz = current_quaternion[..., 3]

    # Extract quaternion rate components (wxyz format)
    dqw = quaternion_rate[..., 0]
    dqx = quaternion_rate[..., 1]
...
def batch_transform_points_inverse(
    ...
    out_points = BatchTransformPointInverse.apply(
        position, quaternion, points, out_points, out_gp, out_gq, out_gpt
    )
    return out_points
...
def pose_to_affine_matrix(
    ...
    """Converts the given pose to a affine transformation matrix (3x4).
    ...
    Returns:
        torch.Tensor: The affine transformation matrix. Shape is (..., 3, 4).
    """
    if out_matrix is None:
        if len(position.shape) == 2:
            out_matrix = torch.zeros(
                (position.shape[0], 3, 4), device=position.device, dtype=position.dtype
            )
        else:
            out_matrix = torch.zeros(
                (position.shape[0], position.shape[1], 3, 4),
                device=position.device,
                dtype=position.dtype,
            )
    out_matrix[..., :3, 3] = position
...
def pose_multiply(
    ...
    if position.shape == position2.shape:
        ...
        log_and_raise("shapes not supported")
...
@wp.kernel
def compute_batch_transform_point_inverse(
    position: wp.array(dtype=wp.vec3),
    quat: wp.array(dtype=wp.vec4),
    pt: wp.array(dtype=wp.vec3),
    n_pts: wp.int32,
    ...
    """A warp kernel to transform batch of points by batch of poses inverse."""
    # given n,3 points and b poses, get b,n,3 transformed points
    # we tile as
    tid = wp.tid()
    b_idx = tid / (n_pts)
    ...
    t_inverse = wp.transform_inverse(t)

    # transform a point
    p = wp.transform_point(t_inverse, in_pt)
...
class TransformPoint(torch.autograd.Function):
    ...
    def forward(
        ...
        pos_v = position.detach().view(-1, 3)
        quat_v = quaternion.detach().view(-1, 4)
        pts_v = points.detach().view(-1, 3)
        out_pts_v = out_points.view(-1, 3)
        check_float32_tensors(pos_v.device, position=pos_v, quaternion=quat_v, points=pts_v, out_points=out_pts_v)

        wp_device, wp_stream = get_warp_device_stream(position)
        wp.launch(
            kernel=compute_transform_point,
            dim=b * n,
            inputs=[
                wp.from_torch(pos_v, dtype=wp.vec3),
                wp.from_torch(quat_v, dtype=wp.vec4),
                wp.from_torch(pts_v, dtype=wp.vec3),
                n,
                b,
            ],
            outputs=[wp.from_torch(out_pts_v, dtype=wp.vec3)],
            stream=wp_stream,
            device=wp_device,
        )
    ...
    def backward(ctx, grad_output):
        # NOTE: grad_output must be contiguous. wp.from_torch rejects 0-stride
        # tensors for vec3 (storage too small for one vec3 read). Autograd can
        # produce 0-stride expanded grads when torch.sum() is applied directly
        # to this Function's output; callers must use mean() or an intermediate
        # op so the gradient is materialized before reaching this backward.
        (
            position,
            quaternion,
            points,
        ...
        grad_v = grad_output.view(-1, 3)
        pos_v = position.view(-1, 3)
        quat_v = quaternion.view(-1, 4)
        pts_v = points.view(-1, 3)
        out_pts_v = out_points.view(-1, 3)
        check_float32_tensors(
            grad_v.device, grad_output=grad_v, position=pos_v, quaternion=quat_v,
            points=pts_v, out_points=out_pts_v,
        )

        wp_adj_out_points = wp.from_torch(grad_v, dtype=wp.vec3)
        ...
        wp.launch(
            ...
            dim=ctx.b * ctx.n,
            inputs=[
                wp.from_torch(pos_v, dtype=wp.vec3, grad=wp_adj_position),
                wp.from_torch(quat_v, dtype=wp.vec4, grad=wp_adj_quat),
                wp.from_torch(pts_v, dtype=wp.vec3, grad=wp_adj_points),
                ctx.n,
                ctx.b,
            ],
            outputs=[
                wp.from_torch(out_pts_v, dtype=wp.vec3, grad=wp_adj_out_points),
            ],
            adj_inputs=[
                None,
                None,
...
class BatchTransformPoint(torch.autograd.Function):
    ...
    def forward(
        ...
        out_pts_v = out_points.view(-1, 3)
        check_float32_tensors(
            pos_v.device, position=pos_v, quaternion=quat_v, points=pts_v, out_points=out_pts_v,
        )
    ...
    @staticmethod
    def backward(ctx, grad_output):
        # NOTE: see TransformPoint.backward for 0-stride vec3 constraint.
        (
            position,
            quaternion,
            points,
        ...
        pts_v = points.view(-1, 3)
        check_float32_tensors(
            grad_v.device, grad_output=grad_v, position=pos_v, quaternion=quat_v, points=pts_v,
        )
        ...
        wp.launch(
            ...
            adjoint=True,
            stream=get_warp_device_stream(grad_output)[1],
            device=get_warp_device_stream(grad_output)[0],
        )
        g_p = g_q = g_pt = None
        if ctx.needs_input_grad[0]:
            g_p = adj_position
...
class BatchTransformPointInverse(torch.autograd.Function):
    """A differentiable function to transform batch of points by a batch of poses."""
    ...
    def forward(
        ...
        wp.launch(
            kernel=compute_batch_transform_point_inverse,
            dim=b * n,
            inputs=[
                wp.from_torch(pos_v, dtype=wp.vec3),
                wp.from_torch(quat_v, dtype=wp.vec4),
            ...
            outputs=[wp.from_torch(out_pts_v, dtype=wp.vec3)],
            stream=get_warp_device_stream(position)[1],
            device=get_warp_device_stream(position)[0],
        )
...
class BatchTransformPose(torch.autograd.Function):
    ...
    def forward(
        ...
        requires_grad = position.requires_grad or quaternion.requires_grad or position2.requires_grad or quaternion2.requires_grad
        if requires_grad:
        ...
        device = position.device
        if quaternion.device != device:
            log_and_raise("quaternion must be on the same device as position")
        if position2.device != device:
            log_and_raise("position2 must be on the same device as position")
        if quaternion2.device != device:
            log_and_raise("quaternion2 must be on the same device as position")
        if out_position.device != device:
            log_and_raise("out_position must be on the same device as position")
        if out_quaternion.device != device:
            log_and_raise("out_quaternion must be on the same device as position")
        ...
        pos_v = position.view(-1, 3)
        quat_v = quaternion.view(-1, 4)
        pos2_v = position2.view(-1, 3)
        quat2_v = quaternion2.view(-1, 4)
        out_pos_v = out_position.view(-1, 3)
        out_quat_v = out_quaternion.view(-1, 4)
        check_float32_tensors(
            device, position=pos_v, quaternion=quat_v, position2=pos2_v, quaternion2=quat2_v,
            out_position=out_pos_v, out_quaternion=out_quat_v,
        )
        ...
        wp.launch(
            ...
            inputs=[
                wp.from_torch(pos_v, dtype=wp.vec3),
                wp.from_torch(quat_v, dtype=wp.vec4),
                wp.from_torch(pos2_v, dtype=wp.vec3),
                wp.from_torch(quat2_v, dtype=wp.vec4),
            ],
            outputs=[
                wp.from_torch(out_pos_v, dtype=wp.vec3),
                wp.from_torch(out_quat_v, dtype=wp.vec4),
            ],
            stream=get_warp_device_stream(position)[1],
            device=get_warp_device_stream(position)[0],
        )
    ...
    def backward(ctx, grad_out_position, grad_out_quaternion):
        ...
        grad_pos_v = grad_out_position.view(-1, 3)
        grad_quat_v = grad_out_quaternion.view(-1, 4)
        pos_v = position.view(-1, 3)
        quat_v = quaternion.view(-1, 4)
        pos2_v = position2.view(-1, 3)
        quat2_v = quaternion2.view(-1, 4)
        ...
        out_quat_v = out_quaternion.view(-1, 4)
        check_float32_tensors(
            grad_pos_v.device, grad_out_position=grad_pos_v, grad_out_quaternion=grad_quat_v,
            position=pos_v, quaternion=quat_v, position2=pos2_v, quaternion2=quat2_v,
            out_position=out_pos_v, out_quaternion=out_quat_v,
        )

        wp_adj_out_position = wp.from_torch(grad_pos_v, dtype=wp.vec3)
        wp_adj_out_quaternion = wp.from_torch(grad_quat_v, dtype=wp.vec4)
        ...
        wp.launch(
            ...
            inputs=[
                wp.from_torch(pos_v, dtype=wp.vec3, grad=wp_adj_position),
                wp.from_torch(quat_v, dtype=wp.vec4, grad=wp_adj_quat),
                wp.from_torch(pos2_v, dtype=wp.vec3, grad=wp_adj_position2),
                wp.from_torch(quat2_v, dtype=wp.vec4, grad=wp_adj_quat2),
            ],
            outputs=[
                wp.from_torch(out_pos_v, dtype=wp.vec3, grad=wp_adj_out_position),
                wp.from_torch(out_quat_v, dtype=wp.vec4, grad=wp_adj_out_quaternion),
            ],
            adj_inputs=[
                None,
                None,
            ...
            adjoint=True,
            stream=get_warp_device_stream(grad_out_position)[1],
            device=get_warp_device_stream(grad_out_position)[0],
        )
...
class TransformPose(torch.autograd.Function):
    ...
    def forward(
        ...
        pos_v = position.detach().view(-1, 3)
        quat_v = quaternion.detach().view(-1, 4)
        pos2_v = position2.detach().view(-1, 3)
        quat2_v = quaternion2.detach().view(-1, 4)
        out_pos_v = out_position.detach().view(-1, 3)
        out_quat_v = out_quaternion.detach().view(-1, 4)
        check_float32_tensors(
            pos_v.device, position=pos_v, quaternion=quat_v, position2=pos2_v, quaternion2=quat2_v,
            out_position=out_pos_v, out_quaternion=out_quat_v,
        )
...
class PoseInverse(torch.autograd.Function):
    ...
    def forward(
        ...
        out_quat_v = out_quaternion.detach().view(-1, 4)
        check_float32_tensors(
            pos_v.device, position=pos_v, quaternion=quat_v,
            out_position=out_pos_v, out_quaternion=out_quat_v,
        )
    ...
    def backward(ctx, grad_out_position, grad_out_quaternion):
        ...
        check_float32_tensors(
            grad_pos_v.device, grad_out_position=grad_pos_v, grad_out_quaternion=grad_quat_v,
            position=pos_v, quaternion=quat_v,
            out_position=out_pos_v, out_quaternion=out_quat_v,
        )
        ...
        wp.launch(
            ...
            adjoint=True,
            stream=get_warp_device_stream(out_position)[1],
            device=get_warp_device_stream(out_position)[0],
        )
        g_p1 = g_q1 = None
        if ctx.needs_input_grad[0]:
            g_p1 = adj_position
...
class QuatToMatrix(torch.autograd.Function):
    ...
    def forward(
        ...
        quat_v = quaternion.detach().view(-1, 4)
        mat_v = out_mat.detach().view(-1, 3, 3)
        check_float32_tensors(quat_v.device, quaternion=quat_v, out_mat=mat_v)
        ...
        wp.launch(
            ...
            outputs=[
                wp.from_torch(mat_v, dtype=wp.mat33),
            ],
            stream=get_warp_device_stream(quaternion)[1],
            device=get_warp_device_stream(quaternion)[0],
        )
    ...
    def backward(ctx, grad_out_mat):
        ...
        grad_mat_v = grad_out_mat.view(-1, 3, 3)
        quat_v = quaternion.view(-1, 4)
        mat_v = out_mat.view(-1, 3, 3)
        check_float32_tensors(grad_mat_v.device, grad_out_mat=grad_mat_v, quaternion=quat_v, out_mat=mat_v)

        wp_adj_out_mat = wp.from_torch(grad_mat_v, dtype=wp.mat33)
        wp_adj_quat = wp.from_torch(adj_quaternion.view(-1, 4), dtype=wp.vec4)
        ...
        wp.launch(
            ...
            outputs=[
                wp.from_torch(mat_v, dtype=wp.mat33, grad=wp_adj_out_mat),
            ],
            adj_inputs=[
                None,
            ],
            ...
            adjoint=True,
            stream=get_warp_device_stream(grad_out_mat)[1],
            device=get_warp_device_stream(grad_out_mat)[0],
        )
        g_q1 = None
        if ctx.needs_input_grad[0]:
            g_q1 = adj_quaternion
...
class MatrixToQuaternion(torch.autograd.Function):
    ...
    def forward(
        ...
        mat_v = in_mat.detach().view(-1, 3, 3)
        out_quat_v = out_quaternion.detach().view(-1, 4)
        check_float32_tensors(mat_v.device, in_mat=mat_v, out_quaternion=out_quat_v)
        ...
        wp.launch(
            ...
            stream=get_warp_device_stream(in_mat)[1],
            device=get_warp_device_stream(in_mat)[0],
        )
    ...
    def backward(ctx, grad_out_q):
        ...
        grad_q_v = grad_out_q.view(-1, 4)
        mat_v = in_mat.view(-1, 3, 3)
        out_quat_v = out_quaternion.view(-1, 4)
        check_float32_tensors(grad_q_v.device, grad_out_q=grad_q_v, in_mat=mat_v, out_quaternion=out_quat_v)

        wp_adj_out_q = wp.from_torch(grad_q_v, dtype=wp.vec4)
        ...
        wp.launch(
            ...
            dim=ctx.b,
            inputs=[
                wp.from_torch(mat_v, dtype=wp.mat33, grad=wp_adj_mat),
            ],
            outputs=[
                wp.from_torch(out_quat_v, dtype=wp.vec4, grad=wp_adj_out_q),
            ],
            adj_inputs=[
                None,
            ],
